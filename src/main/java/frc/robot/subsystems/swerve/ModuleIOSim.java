...
public class ModuleIOSim implements ModuleIO {
    ...
    public void updateInputs(ModuleIOInputs inputs) {
        inputs.driveAppliedDutyCycle = MathUtil
                .clamp(drivePID.calculate(getMetersPerSec(driveMotorSim.getAngularVelocityRadPerSec())), -1, 1);
        ...
        inputs.turnAppliedDutyCycle = MathUtil.clamp(turnPID.calculate(inputs.turnAbsolutePosition.getRotations()), -1,
                1);
        ...
        turnAbsolutePosition = new Rotation2d(
                MathUtil.inputModulus(turnAbsolutePosition.getRadians() + turnAngleDiffRad, -Math.PI, Math.PI));
